...
if (Meteor.isServer) {
  describe('ProfileCollection', function testSuite() {
    ...
    it('#define (duplicate interest)', function test() {
            const testing1 = 'Software Engineering';
            const testing2 = 'Software Engineering';
            const interests = [testing1, testing2];
            const username1 = 'philipmjohnson';
            const defineObject1 = { firstName, lastName, username1, bio, interests, picture, title, github, facebook, instagram };
            expect (function foo() {Profiles.define(defineObject1); }).to.throw(Error);
          });
  });
